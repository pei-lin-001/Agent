...
export type AgentWorkerRole = "researcher" | "coder" | "tester" | "reviewer" | "docWriter";

export interface AgentWorkerPlanHint {
...
export interface AgentDispatchRecommendation extends AgentDispatchDecision {
	...
	workerPlanHints: AgentWorkerPlanHint[];
}
...
const DOC_PATTERN = /文档|文档|docs|documents|documentation|README|readme/i;
...
const MVP_WORKER_PLAN: AgentWorkerPlanHint[] = [
	{ role: "researcher", reason: "Gather context and constraints before implementation." },
	{ role: "coder", reason: "Implement the scoped changes." },
	{ role: "tester", reason: "Run focused validation." },
	{ role: "reviewer", reason: "Review risks, regressions, and missing tests." },
];

const DOC_WORKER_HINT: AgentWorkerPlanHint = {
	role: "docWriter",
	reason: "Update documentation or user-facing notes when the task changes documented behavior.",
};

function buildWorkerPlanHints(mode: AgentDispatchMode, prompt: string): AgentWorkerPlanHint[] {
	if (mode !== "multi_agent_candidate") {
		return [];
	}
	const plan: AgentWorkerPlanHint[] = [...MVP_WORKER_PLAN];
	if (DOC_PATTERN.test(prompt)) {
		plan.push(DOC_WORKER_HINT);
	}
	return plan;
}
...
export function recommendAgentDispatch(prompt: string): AgentDispatchRecommendation {
	...
	return {
		...
		workerPlanHints: buildWorkerPlanHints(decision.mode, prompt),
	};
}

const STEP_DRAFT_TEMPLATES: Record<AgentWorkerRole, Omit<AgentStepDraft, "reason" | "worker">> = {
	researcher: {
		title: "Research context and constraints",
	...
	tester: {
		...
		expectedOutput: "Commands run, pass/fail result, and failure details if any.",
	},
	reviewer: {
		title: "Review risks and regressions",
		expectedOutput: "Findings ordered by severity, with file references when applicable.",
	},
	docWriter: {
		title: "Update documentation",
		expectedOutput: "Updated docs or explanation of why no docs were needed.",
	},
};

function buildStepDraft(hint: AgentWorkerPlanHint): AgentStepDraft {
	const template = STEP_DRAFT_TEMPLATES[hint.role];
	return {
...
export function buildAgentStepDrafts(prompt: string): AgentStepDraft[] {
	const recommendation = recommendAgentDispatch(prompt);
	if (recommendation.action !== "plan_multi_agent") {
		return [];
	}
	return recommendation.workerPlanHints.map(buildStepDraft);
}
